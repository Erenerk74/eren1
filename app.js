...
function createAdminIfNotExists() {
    ...
    const adminExists = users.find(u => u.email === 'admin@kararlab.com');
    
    if (!adminExists) {
        const admin = {
            id: Date.now(),
            name: 'Admin',
            email: 'admin@kararlab.com',
            password: 'admin123',
            type: 'admin'
        };
        users.push(admin);
...
function loadPastReports() {
    ...
    reportsDiv.innerHTML = currentUser.scenarios.map(scenario => `
        <div class="report-card">
            <h4>${scenario.name}</h4>
            <p>Tarih: ${new Date(scenario.date).toLocaleDateString('tr-TR')}</p>
            <p>Sonuç: ${scenario.finalType}</p>
            <button class="btn-secondary btn-small" onclick="viewReport(${scenario.id})">Görüntüle</button>
        </div>
    `).join('');
}
...
function confirmChoice(turn, choice) {
    ...
    if (turn === 1) {
        ...
        showTurnSummary(1, choice, () => loadTurn2());
    } else if (turn === 2) {
        updateIndicatorsAfterTurn2(choice);
        showTurnSummary(2, choice, () => loadTurn3());
    } else if (turn === 3) {
...
function saveScenario(finalType) {
    if (!currentUser || currentUser.type === 'demo') {
        alert('Raporu kaydetmek için giriş yapmalısınız!');
        return;
    }
    ...
    // Rozet kontrolü
    if (finalType === 'Sürdürülebilir Şehir' && !currentUser.badges.includes('green-city')) {
        currentUser.badges.push('green-city');
        alert('🏆 Yeni rozet kazandın: Yeşil Şehir Savunucusu!');
    }
            ...
            <tbody>
                ${users.map(user => `
                    <tr>
                        <td>${user.name}</td>
                        <td>${user.email}</td>
                        <td>${user.type === 'student' ? '🎓 Öğrenci' : '👨‍🏫 Öğretmen'}</td>
